...
Each week fetches fast (~seconds), processes with LLM, saves to JSON.
After all weeks complete, merges results and generates report.

Usage:
  python run_weekly_batches.py --start_date "2026-04-01" --end_date "2026-05-15" --max_records 5000
"""
...
def run_week(start, end, max_records, output_dir):
    ...
    if result.returncode != 0:
        print(f"WARNING: Week {start_str} to {end_str} exited with code {result.returncode}")
        return None
...
def main():
    ...
    parser.add_argument("--max_records", type=int, default=5000, help="Max records per week (default: 5000)")
